...
(cookie=read(cookie))!==undefined)result[name]=cookie}return result};config.defaults={};$.removeCookie=function(key,options){if($.cookie(key)===undefined)return false;$.cookie(key,"",$.extend({},options,{expires:-1}));return!$.cookie(key)}});$(document).ready(function(){if($("#profile_picture_upload").length==0)return;$("#processing_wait").hide();$("#error_text").hide();var person_id=window.location.pathname.match(/\d+/)[0];var url="/image_upload="+person_id+"/";$("#progress .progress-bar").css("width","0%");var csrftoken=$.cookie("csrftoken");set_profile_picture_upload(person_id,url,csrftoken)});function csrfSafeMethod(method){return/^(GET|HEAD|OPTIONS|TRACE)$/.test(method)}
