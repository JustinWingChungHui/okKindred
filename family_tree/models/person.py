...
class PersonManager(models.Manager):
    ...
    def get_related_path(self, user_person, to_person):
        '''
        ...
        found_route = self._search_next_node(relations_by_person, visited_person_ids, route, user_person.id)
        ...
        people_path = list(self.filter(pk__in=found_route))

        #Sort them by the found_route
        people_path.sort(key=lambda p: found_route.index(p.pk))

        return people_path, self._get_path_relations(found_route, relations_by_person)


    def _get_path_relations(self, route, relations_by_person):
        '''
    ...
    def _search_next_node(self, paths_by_person, visited_person_ids, route, end_node_id):
        '''
        ...
        for path in paths_by_person[start_node_id]:
            if not path.to_person_id in visited_person_ids:
                ...
                result = self._search_next_node(paths_by_person, visited_person_ids, new_route, end_node_id)
